...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<c:forEach items="${cartList }" var="cart" varStatus="status"> <!-- varStatus代表的是当前循环遍历对象的状态，index/count/first/end -->
			<%-- index从0开始算，count遍历了几次，first是否是第一次遍历，last是否是最后一次遍历 --%>
		...
		<input type="hidden" name="orderShipping.receiverName" value="caddo"/>
		<input type="hidden" name="orderShipping.receiverMobile" value="13979984297"/>
		<input type="hidden" name="orderShipping.receiverState" value="江西"/>
		<input type="hidden" name="orderShipping.receiverCity" value="南昌"/>
		<input type="hidden" name="orderShipping.receiverDistrict" value="青山湖区"/>
		<input type="hidden" name="orderShipping.receiverAddress" value="蛟桥镇江西农业大学南区"/>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4">caddo</strong>&nbsp;&nbsp;收
							</div>
							<div class="fr">139****4297</div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15">江西省南昌市青山湖区</div>
						<div class="adr-m" limit="30">蛟桥镇经济技术开发区江西农业大学南区</div>
						<div>100000</div>
